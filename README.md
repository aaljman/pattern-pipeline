...
| Value | Behaviour |
| --- | --- |
| `auto` | Use Gemini when `GEMINI_API_KEY` exists, otherwise OpenAI when `OPENAI_API_KEY` exists; provider failures fall back to built-in plans only when the request is still simple enough |
